...
make_repo() {
  ...
  mkdir -p "$repo/src" "$repo/docs" "$repo/.agents/skills/plan-driven-build"
  git -C "$repo" init -q
  ...
  printf '%s\n' 'tracked skill' > "$repo/.agents/skills/plan-driven-build/SKILL.md"
  git -C "$repo" add .
...
mkdir -p "$bootstrap_repo/.agents/skills/framework-update" "$bootstrap_repo/.claude/skills/framework-update"
printf '%s\n' skill > "$bootstrap_repo/.agents/skills/framework-update/SKILL.md"
printf '%s\n' skill > "$bootstrap_repo/.claude/skills/framework-update/SKILL.md"
...
printf '// local skill edit\n' >> "$skill_repo/.agents/skills/plan-driven-build/SKILL.md"
